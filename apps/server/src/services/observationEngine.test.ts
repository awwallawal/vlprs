...
describe('observationEngine — detectRateVariance', () => {
  it('generates observation when has_rate_variance is true', () => {
    const records = [{
    ...
    expect(obs[0].description).toContain('11.110%');
    expect(obs[0].description).toContain('13.33%');
    expect(obs[0].context.dataCompleteness).toBe(100);
    expect(obs[0].context.possibleExplanations).toHaveLength(3);
...
describe('observationEngine — observation templates', () => {
  it('rate variance template uses non-punitive language', () => {
    const records = [{
      ...
      hasRateVariance: true, computedRate: '8.000',
      principal: '100000.00', totalLoan: '108000.00', monthlyDeduction: '3000.00',
      installmentCount: 36, employeeNo: null, sourceFile: 'f.xlsx', sourceSheet: 'S1', sourceRow: 1,
    }];
