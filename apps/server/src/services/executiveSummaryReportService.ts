...
async function buildMonthOverMonthTrend(
  ...
  const now = new Date();
  const currentMonth = currentRecovery?.periodMonth ?? now.getMonth() + 1;
  const currentYear = currentRecovery?.periodYear ?? now.getFullYear();

  // Current period values + previous month snapshot in parallel
  // Note: snapshot is global-only (no MDA scope). MoM comparison is valid because
  // Executive Summary is only generated by SUPER_ADMIN/DEPT_ADMIN (both have null mdaScope).
  const [currentActiveCount, currentExposure, currentCompletionRate, prevRecovery, snapshot] = await Promise.all([
    ...
    getPreviousPeriodRecovery(mdaScope, currentMonth === 1 ? currentYear - 1 : currentYear, currentMonth === 1 ? 12 : currentMonth - 1),
    metricSnapshotService.getPreviousMonthSnapshot(currentYear, currentMonth),
  ]);
  ...
  return {
    activeLoans: buildTrendMetric(currentActiveCount, snapshot ? snapshot.activeLoans : null),
    totalExposure: buildTrendMetric(currentExposureNum, snapshot ? new Decimal(snapshot.totalExposure).toNumber() : null),
    monthlyRecovery: buildTrendMetric(currentRecoveryNum, prevRecoveryNum),
    completionRate: buildTrendMetric(currentCompletionRate, snapshot ? new Decimal(snapshot.completionRate).toNumber() : null),
  };
}

function buildTrendMetric(current: number, previous: number | null): TrendMetric {
  if (previous === null) {
    return { current, previous: null, changePercent: null };
  }
  // previous === 0: can't compute meaningful %; show "No prior data" (matches dashboard "New" handling)
  if (previous === 0) {
    return { current, previous: 0, changePercent: null };
  }
  const changePercent = new Decimal(current).minus(previous).div(previous).mul(100).toDecimalPlaces(1).toNumber();
