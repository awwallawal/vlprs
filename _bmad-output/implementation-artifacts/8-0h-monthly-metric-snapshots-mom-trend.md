...
- [x] [AI-Review][HIGH] H2 (CI fix): `trendMetricSchema` in `reportSchemas.ts` not updated for nullable `previous`/`changePercent` — Zod schema still required `z.number()` while service returns `null`, causing `validateResponse` to reject with 500. Fixed to `z.number().nullable()`. [packages/shared/src/validators/reportSchemas.ts:62-63]
...
- 2026-04-04: CI fix — `trendMetricSchema` in `reportSchemas.ts` not updated for nullable fields. 3 executive-summary integration tests failing with 500 because `validateResponse` rejected `null` values. Fixed `previous`/`changePercent` to `z.number().nullable()`.
...
- `packages/shared/src/validators/reportSchemas.ts` — `trendMetricSchema` fields `previous`/`changePercent` now `.nullable()`
