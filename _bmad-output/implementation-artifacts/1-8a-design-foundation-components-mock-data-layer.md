...
- [ ] Task 1: Install new dependencies (AC: all)
  - [ ] 1.1 Install runtime dependencies: `recharts` (^3.7.0), `date-fns` (^4.1.0), `@fontsource-variable/inter`, `@fontsource-variable/jetbrains-mono`
  - [ ] 1.2 Verify Story 1.6 dependencies exist. If any are missing, install them:
    ```bash
  ...
  - [ ] 1.3 Import fonts in `apps/client/src/main.tsx`: `import '@fontsource-variable/inter'` and `import '@fontsource-variable/jetbrains-mono'` — BEFORE any CSS imports
  - [ ] 1.4 Verify `pnpm typecheck` and `pnpm test` still pass after installs

- [ ] Task 2: Extend design tokens in globals.css (AC: #1, #3)
  - [ ] 2.1 Add non-punitive semantic tokens to `apps/client/src/styles/globals.css` `@theme` block:
    ```css
    ...
    ```
  - [ ] 2.2 Add green-50 token if missing: `--color-green-50: #F0FDF4;`
  - [ ] 2.3 Verify existing font declarations in @theme are correct: `--font-sans: "Inter"...`, `--font-mono: "JetBrains Mono"...` (already present from Story 1.1)
  - [ ] 2.4 Ensure all interactive elements have 44x44px minimum touch target sizing. Add CSS utility if needed: button min-height 40px desktop / 48px mobile, card click areas 44x44px minimum. (NFR-ACCESS-4)

- [ ] Task 3: Extend Badge component with custom variants (AC: #2)
  - [ ] 3.1 Add shadcn/ui Badge if not already added: `npx shadcn@latest add badge`
  - [ ] 3.2 Modify `apps/client/src/components/ui/badge.tsx` — add CVA variants:
  ...
  - [ ] 3.3 Verify badge colour hex values against canonical UX mockup (`ux-design-directions.html`) before finalizing. If discrepancy, use mockup as truth.
  - [ ] 3.4 Create `apps/client/src/components/ui/badge.test.tsx` — test all variants render with correct classes

- [ ] Task 4: Create currency & date formatters (AC: #5)
  - [ ] 4.1 Create `apps/client/src/lib/formatters.ts`:
  ...
  - [ ] 4.2 All financial amount params are `string` (never `number`) — matches API contract where NUMERIC comes as string
  - [ ] 4.3 Create `apps/client/src/lib/formatters.test.ts` — comprehensive edge case tests:
...
- [ ] Task 5: Create NairaDisplay component (AC: #4)
  - [ ] 5.1 Create `apps/client/src/components/shared/NairaDisplay.tsx`:
  ...
  - [ ] 5.3 Create `apps/client/src/components/shared/NairaDisplay.test.tsx`

- [ ] Task 6: Create HeroMetricCard component (AC: #4)
  - [ ] 6.1 Create `apps/client/src/components/shared/HeroMetricCard.tsx`:
  ...
  - [ ] 6.2 Trend indicator: ↑ green for up, ↓ teal for down, → grey for flat (subtle, not prominent)
  - [ ] 6.3 Responsive: full-width on mobile, 4-column grid on desktop (`grid-cols-1 md:grid-cols-2 xl:grid-cols-4`)
  - [ ] 6.4 Create `apps/client/src/components/shared/HeroMetricCard.test.tsx` — test count-up, skeleton, click, reduced motion, all format variants

- [ ] Task 7: Create AttentionItemCard component (AC: #4)
  - [ ] 7.1 Create `apps/client/src/components/shared/AttentionItemCard.tsx`:
  ...
  - [ ] 7.2 "New" state: render 3px teal left-border accent when `isNew` prop is true
  - [ ] 7.3 Empty state component: "No attention items — all systems normal" with green checkmark icon
  - [ ] 7.4 Responsive: full-width list on mobile and tablet; right panel alongside metrics on desktop (lg:)
  - [ ] 7.5 Create `apps/client/src/components/shared/AttentionItemCard.test.tsx` — verify no warning icons, non-punitive language, badge variants, "new" state border, empty state

- [ ] Task 8: Create FileUploadZone component (AC: #4)
  - [ ] 8.1 Create `apps/client/src/components/shared/FileUploadZone.tsx`:
  ...
  - [ ] 8.2 Responsive: full-width on mobile, full-width with drag-drop on tablet, centred 600px max-width on desktop (lg:max-w-[600px] lg:mx-auto)
  - [ ] 8.3 `aria-label="Upload CSV file. Drag and drop or click to browse."` on drop zone
  - [ ] 8.4 Create `apps/client/src/components/shared/FileUploadZone.test.tsx` — test drag events, file selection, error state text verification ("Upload needs attention" not "failed")

- [ ] Task 9: Create MigrationProgressCard component (AC: #4)
  - [ ] 9.1 Create `apps/client/src/components/shared/MigrationProgressCard.tsx`:
  ...
  - [ ] 9.2 Empty state: "No MDAs in migration pipeline" neutral message
  - [ ] 9.3 Responsive: card view on mobile, table row view on desktop (lg:)
  - [ ] 9.4 Create `apps/client/src/components/shared/MigrationProgressCard.test.tsx`

- [ ] Task 10: Create ExceptionQueueRow component (AC: #4)
  - [ ] 10.1 Create `apps/client/src/components/shared/ExceptionQueueRow.tsx`:
  ...
  - [ ] 10.2 Empty state: "No exceptions — all issues resolved" with green checkmark
  - [ ] 10.3 Responsive: full table row on desktop; on mobile, horizontal scroll or compact card collapse
  - [ ] 10.4 Create `apps/client/src/components/shared/ExceptionQueueRow.test.tsx`

- [ ] Task 11: Add shared TypeScript types for mock data (AC: #7)
  - [ ] 11.1 Create `packages/shared/src/types/dashboard.ts`:
    ```typescript
    ...
    ```
  - [ ] 11.2 Create `packages/shared/src/types/mda.ts`:
    ```typescript
    ...
    ```
  - [ ] 11.3 Create `packages/shared/src/types/loan.ts`:
    ```typescript
    ...
    ```
  - [ ] 11.4 Create `packages/shared/src/types/submission.ts`:
    ```typescript
    ...
    ```
  - [ ] 11.5 Create `packages/shared/src/types/exception.ts`:
    ```typescript
    ...
    ```
  - [ ] 11.6 Extend `packages/shared/src/constants/vocabulary.ts` with UI copy constants:
    ```typescript
    ...
    ```
  - [ ] 11.7 Export all new types and `UI_COPY` from `packages/shared/src/index.ts`
  - [ ] 11.8 Run `pnpm --filter shared build` to verify types compile

- [ ] Task 12: Create mock data files (AC: #6)
  - [ ] 12.1 Create `apps/client/src/mocks/dashboardMetrics.ts`:
    ```typescript
  ...
  - [ ] 12.2 Create `apps/client/src/mocks/mdaComplianceGrid.ts` — 63 real Oyo State MDA names with realistic status distribution (45 submitted, 10 pending, 8 overdue)
    ```typescript
    // Target: GET /api/dashboard/compliance
    // Wire: Sprint 5 (Epic 4: Executive Dashboard)
    ```
  - [ ] 12.3 Create `apps/client/src/mocks/attentionItems.ts` — 5 items with non-punitive language:
    ...
    // Wire: Sprint 5 (Epic 4: Executive Dashboard)
    ```
  - [ ] 12.4 Create `apps/client/src/mocks/mdaDetail.ts` — 3 detailed MDAs (Health, Education, Works) with officer names, loan counts, exposure
    ```typescript
    // Target: GET /api/mdas/:id/summary
    // Wire: Sprint 5 (Epic 4: Executive Dashboard)
    ```
  - [ ] 12.5 Create `apps/client/src/mocks/loanDetail.ts` — 5 mock loans across the 3 MDAs with realistic Naira amounts (₦500K-₦5M range), grade level tiers (1-4), status distribution
    ```typescript
    // Target: GET /api/loans/:id
    // Wire: Sprint 2 (Epic 2: Loan Data Management)
    ```
  - [ ] 12.6 Create `apps/client/src/mocks/loanSearch.ts` — 3 search results
    ```typescript
    // Target: GET /api/loans/search
    // Wire: Sprint 2 (Epic 2: Loan Data Management)
    ```
  - [ ] 12.7 Create `apps/client/src/mocks/submissionHistory.ts` — 3 past submissions per MDA with reference numbers (format: "HLT-2026-01-0001")
    ```typescript
    // Target: GET /api/submissions?mdaId=
    // Wire: Sprint 7 (Epic 5: MDA Monthly Submission)
    ```
  - [ ] 12.8 Create `apps/client/src/mocks/migrationStatus.ts` — all 63 MDAs with realistic stage distribution (8 Certified, 15 Validated, 20 Imported, 10 Received, 7 Pending, 3 Reconciled)
    ```typescript
    // Target: GET /api/migration/status
    // Wire: Sprint 4 (Epic 3: Data Migration)
    ```
  - [ ] 12.9 Create `apps/client/src/mocks/exceptionQueue.ts` — 5 exceptions sorted by priority with categories
    ```typescript
    // Target: GET /api/exceptions
    // Wire: Sprint 9 (Epic 7: Exception Management)
    ```
  - [ ] 12.10 Create `apps/client/src/mocks/oyoMdas.ts` — complete list of 63 Oyo State MDA names and codes (used across multiple mock files)

- [ ] Task 13: Create TanStack Query data hooks (AC: #6)
  - [ ] 13.1 Create `apps/client/src/hooks/useDashboardData.ts`:
    ```typescript
    ...
    ```
  - [ ] 13.2 Create `apps/client/src/hooks/useMdaData.ts` — `useMdaComplianceGrid()`, `useMdaDetail(mdaId)`
  - [ ] 13.3 Create `apps/client/src/hooks/useAttentionItems.ts` — `useAttentionItems()`
  - [ ] 13.4 Create `apps/client/src/hooks/useLoanData.ts` — `useLoanDetail(loanId)`, `useLoanSearch(query)`
  - [ ] 13.5 Create `apps/client/src/hooks/useSubmissionData.ts` — `useSubmissionHistory(mdaId)`
  - [ ] 13.6 Create `apps/client/src/hooks/useMigrationData.ts` — `useMigrationStatus()`
  - [ ] 13.7 Create `apps/client/src/hooks/useExceptionData.ts` — `useExceptionQueue()`
  - [ ] 13.8 Each hook:
  ...
  - [ ] 13.9 Create `apps/client/src/hooks/useDashboardData.test.ts` — verify hook returns correct shape via `renderHook` from `@testing-library/react` (with QueryClientProvider wrapper)
  - [ ] 13.10 Create similar test files for each hook (at minimum, verify return type matches expected interface)

- [ ] Task 14: Verify all tests pass (AC: #8)
  - [ ] 14.1 Run `pnpm --filter shared build` — types compile
  - [ ] 14.2 Run `pnpm test` from monorepo root — all workspaces pass
  - [ ] 14.3 Run `pnpm typecheck` — no type errors
  - [ ] 14.4 Run `pnpm lint` — no lint errors
  - [ ] 14.5 Verify component count: 6 shared components + Badge extension + formatters = all tested
  - [ ] 14.6 Verify axe-core accessibility: 0 violations on all component test renders (install `vitest-axe` or `jest-axe` if not already available). Verify all `aria-label` attributes present on interactive components.
  - [ ] 14.7 Verify no `AlertTriangle` or `AlertCircle` imports from lucide-react in any component — only `Info` icon allowed for data-context icons
