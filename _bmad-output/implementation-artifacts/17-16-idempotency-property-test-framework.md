...
**Origin:** SCP 2026-04-15 §4.1 (base). Amended ×2: Addendum 4 §3.3 (published 2026-07-06) + Addendum 3 execution-contract §10.2#3 (harness-as-treaty) → Addendum 5 §3.5 (Kolade/Oke fixtures; folded 2026-07-09). Ledger collision row X-5.
...
**+ A5 §3.5 — three-species golden anchors [P9]:** two canonical cases join the golden regression set (Agreement 15/24 parity), extending harness coverage from below-zero (Species A) to projection and transfer: **Kolade Taiwo Amos** (Species B, frozen at ₦42,498.75 / 5 deductions left / FIRE) via `projection-register.ts`, and **Oke Elizabeth Folashade** (Species C, ₦600k / Agriculture→Education / +₦33,999) via `transfer-restatement.ts`. Both reproduce at pin `667ebdd8`.
...
7. **Given** the Kolade (Species B) and Oke (Species C) canonical anchors, **When** the golden set runs, **Then** both reproduce at pin `667ebdd8` (Kolade via `projection-register.ts`, Oke via `transfer-restatement.ts`) — the harness covers all three finding species, not only below-zero. [A5 §3.5, P9]
...
| 4 | A5 §3.5 (`scp-addendum-5-2026-07-09-DRAFT.md`), folded 2026-07-09 | P9 Kolade (Species B) + Oke (Species C) join the golden set (Agreement 15/24 parity); both reproduce at pin `667ebdd8` |

Evidence keys carried: L2, L4, L6, L7, L11, L12, L19, L20, P9
...
Pending amendments: none — Kolade/Oke fixtures FOLDED 2026-07-09 (A5 §3.5); additions queue to A6
