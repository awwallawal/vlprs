...
editHistory:
  - date: '2026-03-29'
    changes: 'Epic 15 — Beneficiary Onboarding & Verification Pipeline: Extended FR85 from static cross-reference report to temporal onboarding pipeline (month-first-seen tracking, fuzzy matching with confidence bands, monthly scan triggered by submission processing, retirement/deceased verification). Extended FR33(l) onboarding lag attention item with configurable aging threshold (3-month default, 6-month escalation), per-MDA drill-down. Extended FR87(5) No Approval Match observation with temporal context. Added 3 new FRs: FR93 (Committee List Upload Pipeline — two-track wizard: 3-step approval track with 5-column auto-detect and MDA Alias Review with persistent saved mappings, 5-step retiree/deceased track with 17-column schema, three-vector validation, deceased detection via LATE prefix, flexible batch model via approval_batches table), FR94 (Onboarding Pipeline Dashboard — onboarding curve, per-MDA breakdown, historical approval-to-operational rates, Executive Summary stub population), FR95 (Retirement & Deceased Verification Report — feeds existing Pathway 4 gratuityProjectionService for retirees, informational-only for deceased, batch employment event filing with provenance tagging, three-vector validation for 17-column loan creation). Updated MVP Feature Set 14→15 features (added Beneficiary Onboarding & Verification Pipeline). Updated Journey Coverage: Committee — Approval from Phase 2 to Partial MVP. FR count 90→95 (FR91-FR92 added by Epic 11 UAT remediation, FR93-FR95 added by Epic 15). Source: epic-15-beneficiary-onboarding-pipeline.md, epic-15-team-review.md.'
  - date: '2026-03-04'
    changes: 'SQ-1 Analytics Cascade — Executive Dashboard Report Specs restructure: Expanded FR32 with analytics metrics (totalOutstandingReceivables, monthlyCollectionPotential, atRiskAmount, loanCompletionRate) computed via Loan Classification Service with 60-month default window. Expanded FR33 with 5 new attention sources: overdue loan count, stalled deduction count, quick-win opportunities (≤3 installments remaining), dark MDAs (no submission in 6+ months), onboarding lag (approved but not collecting). Expanded FR36 with MDA health score (0-100, weighted formula), health band badges, historical coverage %, staleness indicators, dark MDA labelling. Expanded FR37 Executive Summary with loan portfolio status breakdown, MDA scorecard (top 10 + bottom 5), outstanding receivables ranking, 3-tier recovery potential summary, submission coverage summary. Updated Phase 2 Full Executive Report Suite with Epic 15 scope: Expected vs Actual Exit Gap, Revenue Recovery Planner, Revenue Forecast Waterfall, Concentration Risk, Configurable Window UI, Configurable Health Score Weights, Active Alert Triggers. Zero new FRs — all changes are enrichments of existing FR32/FR33/FR36/FR37. FR count unchanged at 90. Source: executive-dashboard-report-specs.md (restructured from 18-report catalogue to 5 computation services + enriched ACs).'
  - date: '2026-03-01'
...
All 15 features confirmed as must-have after pressure testing:
...
15. **Beneficiary Onboarding & Verification Pipeline** — Upload approved beneficiary and retiree/deceased lists from the Committee via a purpose-built Committee Lists interface (simpler than migration — no column mapping, fixed schemas, two-track wizard). Fuzzy name matching with confidence scoring (high/medium/low bands) against operational loan records. Month-over-month onboarding scan tracking when approved beneficiaries first appear in MDA submissions. Onboarding Pipeline Dashboard with per-MDA drill-down, onboarding curves, and historical approval-to-operational rates. Retirement verification feeding into existing Pathway 4 (gratuity settlement). Deceased verification with informational reporting (no auto-settlement — policy-driven). Feeds attention items (FR33) and observations (FR87).
...
| Committee — Approval | Partial MVP (approval list upload, onboarding tracking, retirement/deceased verification). Full committee workflow remains Phase 2 |
...
- Public Education Portal Expansion — adding interactive features to the MVP Public Zone: EOI form (digital submission with reference number), Approved Beneficiary Lists (searchable, published by batch with NDPR-compliant masked identifiers — the internal onboarding pipeline is delivered in MVP Epic 15, this is the public-facing searchable view), and loan repayment calculator (stakeholder: general public; discovery: Cabinet Presentation). Note: static scheme information pages (eligibility, loan tiers, repayment rules, FAQ) are delivered in MVP Public Zone
...
- Full Executive Report Suite (Epic 15: Executive Analytics & Intelligence) — Expected vs Actual Exit Gap Chart (cumulative gap widening over time), Revenue Recovery Planner (standalone 3-tier drill-down), Revenue Forecast Waterfall (24-month projection tapering as loans complete), Concentration & Exposure Risk View (top 20 individual + top 3 MDA exposure), MDA Scorecard standalone page (full ranked table with all status columns), Rate/Tenure Distribution Dashboard (one-rate-six-tenures model), Configurable Accountability Window UI (24/36/48/60 months, fiscal year, custom), Configurable Health Score Weights, Active Alert Triggers (health drop, new over-deduction, MDA goes dark, quick-win expiring). Depends on E3, E4, E5, E6. (extends Journeys 1-2: AG and Deputy AG; discovery: SQ-1 Legacy CD Analysis, executive-dashboard-report-specs.md)
...
- FR25: Department Admin can upload legacy MDA spreadsheets (.xlsx and .csv) through the migration tool with a column-mapping step to map source columns to required VLPRS fields (up to 10MB / 500 rows per upload). Date of Birth and Date of First Appointment columns are captured when present in source files via intelligent column mapping. Records where either temporal field is absent are flagged as "Profile Incomplete" — migration proceeds but staff profile displays amber indicator until the missing data is provided. These fields feed the retirement computation engine (DOB + 60yrs or appointment date + 35yrs service, whichever comes first)
- FR26: System can validate and categorise migrated records: Clean (< ₦1 — sub-kobo rounding only), Minor Variance (₦1-₦499 — real but small, always surfaced), Significant Variance (≥ ₦500 — demands attention), Structural Error (interest rate not in any known tier), Anomalous (no matching pattern). All variances ≥ ₦1 are surfaced — nothing is hidden from review
...
- FR32: Super Admin can view an executive dashboard with four headline numbers (Active Loans, Total Exposure, Fund Available, Monthly Recovery) without any interaction. Secondary metrics visible without interaction: Pending Early Exits count with expected recovery amount, Gratuity Receivable Exposure total. Additional analytics metrics computed via Loan Classification Service with 60-month default window: Total Outstanding Receivables, Monthly Collection Potential, At-Risk Amount (overdue + stalled outstanding), and Loan Completion Rate (%)
- FR33: Super Admin can view attention items on the dashboard including: (a) MDAs with submission variance >5% for 2+ consecutive months, (b) MDAs with overdue submissions, (c) loans with zero deduction for 60+ days, (d) Auto-Stop Certificates pending MDA acknowledgment, (e) staff with active deductions past computed retirement date — service status verification required, (f) pending early exit computations approaching payment deadline, (g) records missing Staff ID as data quality indicator with percentage complete metric, (h) overdue loan count (loans past expected completion via Loan Classification Service), (i) stalled deduction count (loans with unchanged balance for 2+ consecutive months), (j) quick-win opportunities (loans with ≤3 installments remaining — lowest-effort recoveries), (k) dark MDAs (no submission in 6+ months via Submission Coverage Service), (l) onboarding lag (approved beneficiaries with zero deduction records via Beneficiary Pipeline Service — configurable aging threshold: Amber at 3 months, Gold at 6 months, per-MDA drill-down showing approved count / operational count / awaiting count / average months-to-first-deduction) — with Amber (monitor), Green (informational), Gold (review) status indicators. Each attention item links to the relevant drill-down view for action
...
- FR36: Super Admin can view MDA compliance status (which MDAs have submitted for the current period). Each MDA row displays: submission status (current period), health score (0-100, computed from weighted portfolio performance via MDA Aggregation Service — completion rate ×40, on-track rate ×20, base 40, stall/overdue/over-deduction penalties of -20 each, clamped 0-100), health band badge (healthy ≥70 / attention 40-69 / for-review <40), historical submission coverage percentage, last submission date, and staleness indicator for MDAs with data older than 2 months ("Data as of {date} — {months} months since last update"). MDAs classified as 'dark' (no submission in 6+ months) display an amber "Submission gap observed" label
- FR37: System can generate Executive Summary reports containing: scheme overview (total active loans, total exposure, fund available, monthly recovery rate), MDA compliance status, top 5 variances by magnitude, exception summary (open/resolved counts), month-over-month trend for key metrics, loan portfolio status breakdown (count and % per classification: completed, on-track, overdue, stalled, over-deducted), MDA scorecard (top 10 + bottom 5 by health score), outstanding receivables ranked by MDA (top 10 by exposure), recovery potential summary (3-tier strategy: quick recovery — stalled ≤4mo or overdue ≤6mo; requires intervention — overdue 7-18mo or stalled 5-12mo; extended follow-up — overdue >18mo or stalled >12mo, with amounts and monthly recovery projections per tier), and submission coverage summary (active/spotty/dark MDA counts)
...
- FR85: System can cross-reference monthly deduction records against approved beneficiary lists to produce an Onboarding Pipeline Report with temporal tracking. Report identifies: (a) approved beneficiaries who do not appear in any MDA monthly submission for the corresponding year — flagged as "Approved — Not Yet Operational" with months-since-approval aging, (b) approved beneficiaries who have appeared in MDA submissions — flagged as "Operational" with first-deduction-month tracked, (c) staff appearing in monthly deduction records who are not on any approved beneficiary list — flagged as "Active Deduction — No Approval Record", (d) per-MDA onboarding rates and match rates, (e) retirees/deceased from Committee lists cross-referenced against system loan records for retirement event verification. Cross-reference uses fuzzy Name + MDA matching with confidence bands: High (≥95% name similarity + exact MDA, auto-linked), Medium (85-94% similarity or fuzzy MDA, human review queue), Low (<85%, flagged only). Staff ID auto-upgrades confidence when populated (Epic 13). Monthly scan triggered automatically when MDA submissions are processed — no separate scheduler. Report available to Super Admin and Department Admin. Non-punitive language: "Onboarding Pipeline Report" not "Error Report", findings framed as "requiring verification" not "indicating fault"
...
### Beneficiary Onboarding & Verification Pipeline

- FR93: Department Admin can upload approved beneficiary lists and retiree/deceased lists from the Committee via a Committee Lists section with two upload tracks. Track 1 (Approval/Addendum — 3 steps): 5-column schema auto-detected (S/N, Name, MDA, GL, Amount), MDA Alias Review with persistent saved mappings (confirmed aliases stored in `mda_aliases` table and reused for all future uploads, unknown MDAs block confirmation), batch registration to `approved_beneficiaries` table (separate from loans). Track 2 (Retiree/Deceased — 5 steps): 17-column schema auto-detected by column count and header matching (same structure as BIR migration files — full loan financial data), MDA Alias Review (same component as Track 1), three-vector financial validation (Scheme Expected vs Reverse Engineered vs Committee Declared, reuses prep story 8.0a infrastructure), fuzzy match against existing loan records, confirm & process with individual transactions per record and provenance tagging (`source: RETIREE_LIST_BATCH_IMPORT`). Dual schema detection automatic. Supports list types: APPROVAL, ADDENDUM (appends to existing batch), RETIREE, DECEASED ("LATE" name prefix auto-detected). Flexible batch model: `approval_batches` table with label (required), year (optional — supports ad-hoc batches), notes, uploaded_by. Files with or without header rows handled. Data quality flags (null GL, anomalous amounts) shown as amber indicators, non-blocking. Upload available to Super Admin and Department Admin. Non-punitive: "X beneficiaries registered" not "X records imported"
- FR94: Super Admin and Department Admin can view an Onboarding Pipeline Dashboard showing: (a) headline metrics — total approved (all batches), total operational, total awaiting, overall operational rate (%), (b) onboarding curve — line/area chart showing % operational over time since batch approval date with one line per batch year, (c) per-MDA breakdown table — MDA name, approved count, operational count, awaiting count, operational rate (%), average months-to-first-deduction, sortable by any column, (d) individual beneficiary drill-down with timeline (approved → awaiting → first deduction → current status) and matched loan details, (e) historical approval-to-operational rates per MDA across batch years, (f) batch selector for year filtering. Dashboard accessible on its own page AND populates the currently-stubbed Onboarding Pipeline Summary section in the Executive Summary report (FR37) with: total approved, total operational, operational rate, top 5 MDAs with longest average onboarding time, count of beneficiaries awaiting >3 months. Exportable as branded PDF with Oyo State crest. Non-punitive: "not yet operational" not "missing", "awaiting first deduction" not "non-compliant MDA", per-MDA rates as factual averages not rankings
- FR95: Super Admin and Department Admin can generate a Retirement & Deceased Verification Report by cross-referencing uploaded retiree/deceased lists against system loan records. Two categories with distinct pathways: (a) RETIRED — matched retirees feed into existing Pathway 4 (gratuity settlement via `gratuityProjectionService`), Department Admin can batch-file RETIRED employment events for high-confidence matches triggering automatic gratuity receivable computation. (b) DECEASED ("LATE" prefix) — matched deceased trigger DECEASED loan status (terminal), outstanding balance recorded for visibility, no automated settlement pathway (estate/guarantor recovery is policy-driven, outside VLPRS scope). Report shows five verification statuses: matched with event recorded (confirmed), matched with no event (action required), matched with stalled balance and no event (high priority), no match with 17-column financial data (offer loan creation via three-vector validated data + immediate RETIRED/DECEASED status), no match without financial data (connects to onboarding pipeline). Each batch-imported event includes provenance: `source: RETIREE_LIST_BATCH_IMPORT`, `uploadReference: <upload_id>`, `batchDate: <import_date>`. Individual transactions per record (not batch — prevents deadlocks, allows partial success). Hard dependency on prep story 8.0a (three-vector model) for processing retiree financial data. Non-punitive: "verification status" not "processing failures", "estate/guarantor recovery — policy determination required" not "unrecoverable"

### E8 Retro — UAT-Driven Requirements (2026-04-06)

- FR96: MDA Officers can upload migration data for their own MDA using the same Excel upload pipeline as the AG/Department Admin (three-vector validation, period detection, column mapping, variance categorisation). MDA-initiated uploads are marked with source `mda_upload` and enter `pending_verification` status — they are NOT auto-baselined. Department Admin or AG reviews the upload quality score and either approves (proceeds to selective baseline) or returns to MDA with notes. Coverage Tracker displays upload source attribution per MDA per month. All 63 MDAs can contribute data independently. Non-punitive: "pending verification" not "unapproved"
- FR97: MDA Officers have a purpose-built dashboard showing: (a) hero metrics scoped to their MDA (active loans, monthly recovery, outstanding receivables, completion rate) — all clickable with drill-down, (b) migration status section (records uploaded, unique staff migrated, quality score with variance category breakdown, mini Coverage Tracker for their MDA), (c) action items section (flagged records count, review deadline, direct links to correction worksheet), (d) pre-submission checkpoint (approaching retirement, zero deduction review as paginated table, pending events), (e) gap-aware submission history (months with data show source/status, months without data show "Gap" with upload action), (f) quick actions bar (Upload Migration Data, Submit Monthly Return, File Event). Sidebar: My Dashboard, Upload Data, Submit Monthly, My Reviews, Employment Events, Reconciliation, My Reports
- FR98: Public certificate verification page accessible without authentication at `/verify/:certificateId`. Displays: Oyo State Government branding, verification status (verified/not found), certificate ID, beneficiary name, MDA, completion date. Includes manual entry field for certificate ID (for verification without QR scan). QR codes on printed certificates link to this page
- FR99: Every visible metric, count, or amount across all dashboards and reports must be clickable and navigate to a drill-down showing the records behind the number. Team agreement #11: "Every number is a doorway." Applies to: Recovery Potential Summary cards, MDA Scorecard table rows, Operations Hub cards, metric drill-down pages (add "View All Loans" shortcut), Zero Deduction Review (paginated table, not plain text list), Top Variances (paginated, not limited to 5)
- FR100: Upload supersede workflow includes record-level data comparison before confirmation: matched records (unchanged), modified records (field-level diffs highlighted), new records (in new file only), removed records (in old file only). AG/Department Admin reviews comparison and confirms replacement. Correction reasons mandatory for ALL record corrections (not just flagged records) — enforced in RecordDetailDrawer for all roles
- FR101: MDA Officers can view an MDA-scoped Beneficiary Ledger showing loan lifecycle: active staff, completed loans (with certificate status), transferred-out staff (destination MDA, date), transferred-in staff (source MDA, date), and consecutive loan history. Uses existing `beneficiaryLedgerService` with `withMdaScope`, `transfers` and `employment_events` tables for transfer history
- FR102: Within-file deduplication detection: when the same person (matched by staffName + employeeNo) appears twice in the same upload file for the same period, the system flags the duplicate before baseline creation. Resolution options: merge records, keep one, or flag for manual review. Complements existing cross-MDA deduplication (FR90/Story 3.8)
